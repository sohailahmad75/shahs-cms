...
// src/pages/products/components/ProductFormDrawer.tsx
...
const getDefaultValues = (type: string) => ({
    ...
    itemCode: '',
    category: '',
    initialQty: type === 'stock' ? '' : undefined,
    asOfDate: type === 'stock' ? '' : undefined,
    reorderPoint: type === 'stock' ? '' : undefined,
    stockAssetAccount: 'Stock Asset',
    description: '',
    salesPrice: '',
    incomeAccount: 'Sales of Product Income',
    ...
    return (
        <div className={`p-6 h-full overflow-y-auto ${finalDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-white'}`}>
            ...
            <div className="flex items-center justify-between mb-6 pb-4 border-b">
                ...
                <span
                    className={`transition duration-200 ease-in-out hover:scale-110 cursor-pointer ${finalDarkMode ? 'text-slate-100 hover:text-slate-200' : 'text-gray-600 hover:text-orange-500'}`}
                    onClick={onClose}
                    role="button"
                    aria-label="Close drawer"
                >
            ...
            >
                {({ values, errors, touched, handleChange, setFieldValue, handleSubmit }) => (
                    <Form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        />
                                        {touched.initialQty && errors.initialQty && (
                                            <p className="mt-1 text-sm text-red-500">{errors.initialQty}</p>
                                        )}
                                    </div>
                                    <div>
                                        <label className={`block text-sm font-medium ${finalDarkMode ? 'text-slate-300' : 'text-gray-700'}`}>
                        ...
                        {/* Cost & Expense Account (Show for Stock & Non-stock) */}
                        {(showStockFields || showNonStockFields) && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className={`block text-sm font-medium ${finalDarkMode ? 'text-slate-300' : 'text-gray-700'}`}>
