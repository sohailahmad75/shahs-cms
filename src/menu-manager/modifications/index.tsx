...
const ModifierList: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetModifiersQuery(menuId);
  const [showModal, setShowModal] = useState(false);
  const [deleteModifier, { isLoading: deletingMod }] =
    useDeleteModifierMutation();
              ...
              >
                ...
                <div className="absolute top-3 right-3 flex gap-2">
                  ...
                  <ConfirmDelete
                    loading={deletingMod}
                    onConfirm={async () => {
                      await deleteModifier(mod.id).unwrap();
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <h3
                      className={`text-lg font-semibold ${isDarkMode ? "text-white" : "text-orange-100"} capitalize truncate`}
                    >
                      {mod.name}
                    </h3>
                    {mod.description && (
                      <p
                        className={`text-sm ${isDarkMode ? "text-white" : "text-gray-600"} mt-1 line-clamp-2`}
                      >
                        {mod.description}
                      </p>
                    )}
                    ...
                    <div
                      className={`mt-3 text-sm ${isDarkMode ? "text-slate-500" : "text-gray-700"} space-y-1`}
                    >
                      <p>
                  ...
                  {itemNames.length > 0 && (
                    <div className="flex flex-col justify-between">
                      <div
                        className={`text-sm ${isDarkMode ? "text-slate-500" : "text-gray-700"}`}
                      >
