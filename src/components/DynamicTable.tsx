...
export function DynamicTable<T>({
  ...
  const EmptyState = (
    ...
    >
      <p
        className={`text-lg font-medium mb-2 ${isDarkMode ? "text-slate-300" : "text-slate-800"}`}
      >
        {emptyStateMessage}
      </p>
      <p
  ...
  return (
    <div>
      ...
      <div className="md:hidden space-y-4">
        {data.length === 0 ? (
          <div
            className={`rounded-lg border p-6 text-center ${isDarkMode ? "border-slate-700 bg-slate-900 text-slate-400" : "border-slate-200 bg-white text-slate-500"}`}
          >
            {EmptyState}
          </div>
        ) : (
          data.map((row, index) => (
            <div
              key={String(row[rowKey] as unknown as string)}
              className={`rounded-lg border p-4 ${isDarkMode ? "border-slate-700 bg-slate-900" : "border-slate-200 bg-white"}`}
            >
              {columns.map((col, idx) => (
